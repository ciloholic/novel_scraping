...
module NovelScraping
  class Nocturne < BaseScraper
    ...
    class << self
      def get_site(url)
        top_html = Nokogiri::HTML(NovelScraping.uri_open(url, { cookie: 'over18=yes', user_agent: user_agent }))
        ...
        htmls = [top_html].tap do |html|
          ...
          if path.present?
            ...
            [*2..last_page].each do |page|
              html << Nokogiri::HTML(NovelScraping.uri_open("#{url}/?p=#{page}", { cookie: 'over18=yes', user_agent: user_agent }))
            end
          end
        end
